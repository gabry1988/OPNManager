<script lang="ts">
    ...
    function toggleInterface(interfaceName: string) {
        if (hiddenInterfaces.has(interfaceName)) {
            hiddenInterfaces.delete(interfaceName);
        } else {
            hiddenInterfaces.add(interfaceName);
        }
        drawCharts();
    }
    ...
    function updateChartDimensions() {
        if (chartContainer && canvasIn && canvasOut) {
            chartWidth =
                chartContainer.clientWidth || chartContainer.offsetWidth;

            if (!chartWidth || chartWidth < 10) {
                console.log(
                    "Chart width not properly detected, using fallback",
                );
    ...
    function processAndAddData(data) {
        ...
        if (dataCollection.length > maxPoints) {
            dataCollection = dataCollection.slice(
                dataCollection.length - maxPoints,
            );
        }
    ...
    function drawSingleInterfaceArea(
        ...
        dataCollection.forEach((dataPoint, index) => {
            ...
            if (dataPoint.byInterface[interfaceName]) {
                value =
                    direction === "in"
                        ? dataPoint.byInterface[interfaceName]
                              .bits_per_second_in
                        : dataPoint.byInterface[interfaceName]
                              .bits_per_second_out;
            }
            ...
            const x =
                margin.left +
                (index / (dataCollection.length - 1)) * graphWidth;
            const y =
                margin.top + graphHeight - (value / maxBits) * graphHeight;
                ...
                <div class="flex flex-wrap gap-3 min-h-8">
                    {#if interfaces.length > 0 && !isLoadingNewData}
                        {#each interfaces as interfaceName}
                            <div
                                class="flex items-center px-2 py-1 rounded-lg hover:bg-base-200 cursor-pointer transition-colors"
                                on:click={() => toggleInterface(interfaceName)}
                                title={hiddenInterfaces.has(interfaceName)
                                    ? `Show ${interfaceName}`
                                    : `Hide ${interfaceName}`}
                            >
                                <div
                                ...
                                <span
                                    ...
                                    style="opacity: {hiddenInterfaces.has(
                                        interfaceName,
                                    )
                                        ? '0.4'
                                        : '1'};"
                                ...
                                >
                                    <path
                                        fill="currentColor"
                                        d={hiddenInterfaces.has(interfaceName)
                                            ? mdiEyeOffOutline
                                            : mdiEyeOutline}
                                    />
                                </svg>

<script lang="ts">
    ...
    import { mdiArrowUp, mdiArrowDown, mdiEyeOffOutline, mdiEyeOutline } from '@mdi/js';
    ...
    function toggleInterface(interfaceName: string) {
      console.log(`Before toggle: ${interfaceName} hidden: ${hiddenInterfaces.has(interfaceName)}`);
      ...
      console.log(`After toggle: Hidden interfaces: ${Array.from(hiddenInterfaces).join(', ')}`);
      setTimeout(() => {
        drawCharts();
      }, 0);
    }
    ...
    async function fetchTrafficData() {
      try {
        ...
        if (!data || data.length === 0) {
          console.log("No traffic data available yet, waiting for more samples...");
          return;
        }
    ...
    function updateChartDimensions() {
      if (chartContainer && canvasIn && canvasOut) {
        ...
        console.log("Chart dimensions updated - width:", chartWidth, "height:", chartHeight);
      } else {
        console.log("Cannot update chart dimensions - some elements not initialized yet");
      }
    }
    ...
    function processAndAddData(data) {
      ...
      data.forEach(point => {
        ...
        if (!latestDataByInterface[interfaceName] || 
    ...
    function drawCharts() {
      if (!canvasIn || !canvasOut || chartWidth === 0 || inChartHeight === 0 || outChartHeight === 0) {
        return;
      }
    ...
    function drawTrafficChart(canvas, height, direction, maxBits) {
      ...
      interfaces.forEach(interfaceName => {
        ...
        drawSingleInterfaceArea(ctx, interfaceName, direction, color, margin, graphWidth, graphHeight, maxBits);
      });
    }
    
    function drawSingleInterfaceArea(ctx, interfaceName, direction, color, margin, graphWidth, graphHeight, maxBits) {
      ...
      dataCollection.forEach((dataPoint, index) => {
        ...
        const x = margin.left + (index / (dataCollection.length - 1)) * graphWidth;
    ...
    onDestroy(() => {
      ...
      if (dataLoadTimeoutId) clearTimeout(dataLoadTimeoutId);
                ...
                {#key [interfaces, JSON.stringify(Array.from(hiddenInterfaces))]}
                  {#each interfaces as interfaceName}
                    <button 
                      ...
                      data-hidden={hiddenInterfaces.has(interfaceName) ? "true" : "false"}
                    >
                      <!-- Color dot with forced visibility -->
                      <div 
                        class="w-3 h-3 rounded-full mr-2 flex-shrink-0" 
                        style="background-color: {getInterfaceColor(interfaceName)}; 
                               filter: {hiddenInterfaces.has(interfaceName) ? 'grayscale(80%) brightness(50%)' : 'none'};
                      ...
                      <span 
                        ...
                        style="text-decoration: {hiddenInterfaces.has(interfaceName) ? 'line-through' : 'none'} !important;
                               color: {hiddenInterfaces.has(interfaceName) ? '#6b7280' : 'inherit'} !important;
                      ...
                      <span class="ml-1 text-xs flex-shrink-0" 
                            style="color: {hiddenInterfaces.has(interfaceName) ? '#ef4444' : 'inherit'} !important;
                                   opacity: {hiddenInterfaces.has(interfaceName) ? '1' : '0.5'} !important;
                                   display: inline !important;">
          ...
          <div class="flex justify-center items-center h-80">
            <div class="text-center">
              ...
              <p class="mt-4 text-base-content">
                {isFirstDataLoad ? "Collecting initial traffic data..." : "Refreshing traffic data..."}
              </p>
            </div>
          </div>
        {:else}
          <div>
            ...
            <canvas bind:this={canvasIn} style="width: 100%; height: {chartHeight}px;"></canvas>
          </div>
          <div>
            <div class="flex items-center">
            ...
            <canvas bind:this={canvasOut} style="width: 100%; height: {chartHeight}px;"></canvas>
          </div>
        {/if}
      </div>
    </div>
